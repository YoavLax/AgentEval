#!/usr/bin/env python3
"""GitHub Action entrypoint for AgentEval.

Runs AgentEval with JSON output, emits PR annotations via workflow
commands, writes a job summary to $GITHUB_STEP_SUMMARY, and exits with
AgentEval's exit code.
"""
...
def _build_command() -> list[str]:
    """Build the AgentEval CLI invocation from INPUT_* env vars."""
    path = os.environ.get("INPUT_PATH", ".")
    cmd = ["agenteval", path, "--format", "json", "--no-color"]
...
def _emit_annotations(data: dict) -> None:
    ...
    for file_result in data.get("results", []):
        ...
        for diag in file_result.get("diagnostics", []):
            ...
            print(f"::{gh_level} {loc},title=AgentEval: {rule}::{safe_msg}")
...
def _write_summary(data: dict) -> None:
    ...
    if failed == 0:
        lines.append(f"### ✅ AgentEval: {passed}/{total} files passed\n")
    else:
        lines.append(f"### ❌ AgentEval: {failed}/{total} files failed\n")
...
def _set_outputs(data: dict, exit_code: int) -> None:
    ...
    with open(output_path, "a") as f:
        ...
        f.write("json<<AGENTEVAL_JSON_EOF\n")
...
def main() -> None:
    ...
    if result.returncode == 2:
        ...
        print(f"::error::agenteval: {msg}")
        sys.exit(2)

    try:
        data = json.loads(result.stdout)
    except (json.JSONDecodeError, ValueError):
        print("::error::agenteval produced unexpected output")
        if result.stdout:
            print(result.stdout)
        if result.stderr:
